...
# OR Build servers individually
cd model-server && mvn clean verify
cd ../glsp-server && mvn clean verify
```

> **_Remark:_**&nbsp; Please make sure to build the `model-server` component prior to the `glsp-server` component as it consumes it as dependency.
...
- `Launch All TaskList Applications [DEBUG]`<br>
  This is a convenience compound config that launches all DEBUG relevant launch configs:
  ...
  - `Launch Theia Frontend [DEBUG]`
    - The TaskList Model Server process, the TaskList GLSP server process, the TaskList Theia backend in external server mode and the Theia frontend Chrome instance. Enables debugging of both the client, glsp-server and model-server code simultaneously.
